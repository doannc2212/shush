...
{#if mutedTabs.length === 0}
  <div class="empty">
    ...
    <p class="empty-body">
      Use the buttons above to shush your current tab, or pick specific tabs from the Mute tabs
      view. We won't judge how many you silence.
    </p>
  </div>
{:else}
  <ul class="list">
    {#each mutedTabs as tab (tab.tabId)}
      <li class="item">
        ...
        <button type="button" onclick={() => handleUnmute(tab.tabId)} class="btn-unmute">
          Let it speak
        </button>
      </li>
    {/each}
